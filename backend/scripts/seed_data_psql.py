#!/usr/bin/env python3
"""
Script para poblar la base de datos PostgreSQL con datos de prueba.

Ejecutar con: uv run python scripts/seed_data_psql.py

Este script crea usuarios de ejemplo para desarrollo y testing.
Compatible con PostgreSQL (usa datetime naive sin timezone).
...
async def create_fichajes(session, users: dict[str, User]) -> list[Fichaje]:
    ...
    created_fichajes = []
    # Usar datetime con timezone UTC para PostgreSQL
...
async def seed_database(clear: bool = True) -> None:
    ...
    """
    print("=" * 80)
    print("🌱 SEED DATA - Sistema de Gestión de Fichajes y RRHH (PostgreSQL)")
    print("=" * 80)
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Poblar base de datos PostgreSQL con datos de prueba"
    )
    parser.add_argument(
        "--clear",
        action="store_true",
